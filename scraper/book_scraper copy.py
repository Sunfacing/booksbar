...
from eslite import es_scrap_category, es_remove_duplicates, es_checking_new_unfound_products, es_scrap_new_products, es_scrap_unfound_products, es_drop_old_collection
...
with DAG(
dag_id='b_the_book_scraper_copy',
...
tags=['final_version'],
    ...
    task_e1 = PythonOperator(task_id='es_scrap_category', python_callable=es_scrap_category)
    task_e2 = PythonOperator(task_id='es_remove_duplicates', python_callable=es_remove_duplicates)
    task_e3 = PythonOperator(task_id='es_checking_new_unfound_products', python_callable=es_checking_new_unfound_products)
    task_e4 = PythonOperator(task_id='es_scrap_new_products', python_callable=es_scrap_new_products, retries = 300)
    task_e5 = PythonOperator(task_id='es_scrap_unfound_products', python_callable=es_scrap_unfound_products)
    task_e6 = PythonOperator(task_id='es_drop_old_collection', python_callable=es_drop_old_collection)
    ...
    [task_k1 >> task_k2 >> task_k3 >> task_k4 >> task_k5 >> task_k6 , task_m1 >> task_m2 >> task_m3 >> task_m4 >> task_m5 >> task_m6, task_e1 >> task_e2 >> task_e3 >> task_e4 >> task_e5 >> task_e6] >> task_s1 >> task_s2 >> task_s3 >> task_s4 >> task_s5
